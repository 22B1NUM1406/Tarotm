...
import { checkActivePayment } from './utils/qpayService';
...
const TarotApp = () => {
  ...
  const showMessage = (type, msg) => {
    if (type === 'error') { setError(msg);   setSuccess(''); setTimeout(() => setError(''),   5000); }
    else                  { setSuccess(msg); setError('');   setTimeout(() => setSuccess(''), 5000); }
  };
  ...
  const handleTopicSelect = (topic) => {
    ...
    setLoading(true);
    setTimeout(() => {
      setShuffledCards([...TAROT_CARDS].sort(() => Math.random() - 0.5));
      setSelectedCards([]);
      setLoading(false);
      navigateTo('cards');
    }, 1000);
  ...
  const handleCardSelect = (cardId) => {
    if (selectedCards.length < 3 && !selectedCards.includes(cardId)) {
      ...
      if (next.length === 3) {
        setTimeout(() => {
          setLoading(true);
          setTimeout(() => { setLoading(false); navigateTo('result'); }, 1000);
        }, 1000);
      }
    }
  };
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'home':
        return (
          <Home
            user={user} email={email} setEmail={setEmail}
            ...
            navigateTo={navigateTo} error={error} success={success}
            authLoading={authLoading} activeTab={activeTab}
      ...
      case 'cards':
        return (
          <Cards
            shuffledCards={shuffledCards} selectedCards={selectedCards}
      ...
      case 'result':
        return (
          <Result
            selectedCards={selectedCards} selectedTopic={selectedTopic}
            tarotCards={TAROT_CARDS} resetReading={resetReading}
      ...
      case 'blogs': return <Blogs onArticleClick={(a) => setSelectedArticle(a)} />;
      case 'news':  return <News  onArticleClick={(a) => setSelectedArticle(a)} />;
      case 'about': return <About professionalReadingLink={PROFESSIONAL_READING_LINK} navigateTo={navigateTo} />;
      default: return <Home />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-950 via-indigo-950 to-purple-950 text-white relative overflow-hidden">
      <Starfield />
      <Navbar
        user={user} currentPage={currentPage} navigateTo={navigateTo} hasPaid={hasPaid}
        handleLogout={() => handleLogout(showMessage, setUser, setEmail, setPassword, setPageHistory, setCurrentPage)}
      />
      <div className="pt-16 relative z-10">{renderPage()}</div>
      ...
      {(success || error) && (
        <div className="fixed bottom-8 right-8 z-50 animate-slide-up">
          <div className={`px-6 py-4 rounded-xl shadow-2xl backdrop-blur-md ${
            error ? 'bg-red-500/90 border border-red-400' : 'bg-green-500/90 border border-green-400'}`}>
      ...
      {selectedArticle && (
        <DetailedArticleModal article={selectedArticle} onClose={() => setSelectedArticle(null)} />
      )}
